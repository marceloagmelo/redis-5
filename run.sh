...
REDIS_MASTER_SERVICE=${REDIS_MASTER_SERVICE:-'redis-master'}
...
function launchmaster() {
  ...
  # Se existir o arquivo de configuracao gerado pelo configmap
  if [ -f /var/lib/redis/redis-master/redis.conf.template ]; then
     log_info 'Atualizando arquivo de conf [/var/lib/redis/redis-master/redis.conf] ...'
     envsubst < /var/lib/redis/redis-master/redis.conf.template > /var/lib/redis/redis-master/redis.conf
  fi
...
function launchsentinel() {
  echo "Starting sentinel ${REDIS_SENTINEL_SERVICE}..."
  
  #redis-cli -h ${REDIS_MASTER} INFO
  while true; do
    master=$(timeout 5 ${REDIS_PREFIX}/bin/redis-cli -h ${REDIS_SENTINEL_SERVICE} -p ${REDIS_SENTINEL_SERVICE_PORT} --csv SENTINEL get-master-addr-by-name mymaster | tr ',' ' ' | cut -d' ' -f1)
    if [[ -n ${master} ]]; then
      master="${master//\"}"
    else
      echo "Failed to find master."
      sleep 10
       master=${REDIS_MASTER_SERVICE}
    fi
    echo "Using ${master} as master"
...
function launchslave() {
  echo "Starting slave with sentinel ${REDIS_SENTINEL_SERVICE}..."
  ...
  # Se existir o arquivo de configuracao gerado pelo configmap
  if [ -f /var/lib/redis/redis-slave/redis.conf.template ]; then
     log_info 'Atualizando arquivo de conf [/var/lib/redis/redis-slave/redis.conf] ...'
     envsubst < /var/lib/redis/redis-slave/redis.conf.template > /var/lib/redis/redis-slave/redis.conf
  fi
